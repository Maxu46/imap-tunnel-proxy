#!/usr/bin/env python3
"""
IMAP Tunnel Server - Fast Binary Mode

Version: 1.0.0

Protocol:
1. IMAP handshake (CAPABILITY, STARTTLS, LOGIN) - looks like real IMAP
2. After LOGIN success, switch to binary streaming mode
3. Full-duplex binary protocol - no more IMAP overhead
...
logger = logging.getLogger('imap-tunnel-server')


# ============================================================================
# Binary Protocol (used after IMAP handshake)
# ============================================================================
...
class TunnelSession:
    ...
    async def run(self):
        ...
        try:
            # Phase 1: IMAP handshake
            if not await self._imap_handshake():
                return
    ...
    async def _imap_handshake(self) -> bool:
        """Do IMAP handshake - this is what DPI sees."""
        try:
            # Send IMAP greeting
            await self._send_line(f"* OK {self.config.hostname} IMAP4rev1 Service Ready")

            # Wait for CAPABILITY command
            ...
            if not cmd.startswith('CAPABILITY'):
                await self._send_line(f"{tag} BAD Unknown command")
                return False

            # Send capabilities (pre-TLS)
            await self._send_line("* CAPABILITY IMAP4rev1 STARTTLS LOGINDISABLED")
            ...
            await self._send_line(f"{tag} OK Begin TLS negotiation now")

            # Upgrade to TLS
            await self._upgrade_tls()

            # Wait for CAPABILITY again (post-TLS)
            line = await self._read_line()
            if not line:
                return False
            ...
            # Send capabilities (post-TLS, with AUTH)
            await self._send_line("* CAPABILITY IMAP4rev1 AUTH=PLAIN IDLE NAMESPACE UIDPLUS MOVE ID LITERAL+ BINARY")
            await self._send_line(f"{tag} OK CAPABILITY completed")

            # Wait for AUTHENTICATE PLAIN
            line = await self._read_line()
            if not line:
                return False
            ...
            if cmd == 'AUTHENTICATE' and len(parts) >= 3:
                auth_parts = parts[2].split(' ', 1)
                if auth_parts[0].upper() != 'PLAIN':
                    await self._send_line(f"{tag} NO Unsupported authentication mechanism")
                    return False

                # Check if token is inline or needs continuation
                if len(auth_parts) > 1:
                    token = auth_parts[1]
                else:
                    # Send continuation request
                    await self._send_line("+")
                    token_line = await self._read_line()
                    if not token_line:
                        return False
                    token = token_line
            elif cmd == 'LOGIN' and len(parts) >= 3:
                # LOGIN command with token as "password"
                # Format: LOGIN username token
                login_parts = parts[2].split(' ', 1)
                if len(login_parts) < 2:
                    await self._send_line(f"{tag} NO LOGIN failed")
                    return False
                token = login_parts[1].strip('"')
            else:
                await self._send_line(f"{tag} BAD Unknown command")
                return False
            ...
            if not valid or not username:
                ...
                await self._send_line(f"{tag} NO [AUTHENTICATIONFAILED] Authentication failed")
                return False
            ...
            await self._send_line(f"{tag} OK [CAPABILITY IMAP4rev1 IDLE NAMESPACE] Logged in")
...
class TunnelServer:
    ...
    async def start(self):
        ...
        logger.info(f"IMAP Tunnel Server on {addr[0]}:{addr[1]}")
...
def main():
    parser = argparse.ArgumentParser(description='IMAP Tunnel Server')
    ...
    config = ServerConfig(
        ...
        port=server_conf.get('port', 993),
        hostname=server_conf.get('hostname', 'imap.example.com'),
    ...
    if not users:
        ...
        logger.error("Use imap-tunnel-adduser to add users")
        return 1
