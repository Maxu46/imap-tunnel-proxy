#!/usr/bin/env python3
"""
Generate self-signed TLS certificates for IMAP tunnel.
...
def generate_ca_certificate(
    ...
    common_name: str = "IMAP Tunnel CA",
    days_valid: int = 3650
) -> x509.Certificate:
    ...
    subject = issuer = x509.Name([
        ...
        x509.NameAttribute(NameOID.ORGANIZATION_NAME, "IMAP Tunnel"),
...
def generate_server_certificate(
    ...
    san = x509.SubjectAlternativeName([
        ...
        x509.DNSName(f"imap.{hostname.split('.', 1)[-1] if '.' in hostname else hostname}"),
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate TLS certificates for IMAP tunnel'
    )
    parser.add_argument(
        '--hostname',
        default='imap.example.com',
        help='Server hostname for certificate (default: imap.example.com)'
    )
    parser.add_argument(
        '--output-dir',
        default='.',
