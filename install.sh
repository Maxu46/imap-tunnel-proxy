#!/bin/bash
#
# IMAP Tunnel Proxy - Server Installation Script
#
...
# GitHub raw URL base
GITHUB_RAW="https://raw.githubusercontent.com/x011/imap-tunnel-proxy/main"

# Installation directories
INSTALL_DIR="/opt/imap-tunnel"
CONFIG_DIR="/etc/imap-tunnel"
BIN_DIR="/usr/local/bin"

# Files to download
PYTHON_FILES="server.py client.py common.py generate_certs.py"
SCRIPTS="imap-tunnel-adduser imap-tunnel-deluser imap-tunnel-listusers imap-tunnel-update"
...
install_systemd_service() {
    ...
    cat > /etc/systemd/system/imap-tunnel.service << EOF
[Unit]
Description=IMAP Tunnel Proxy Server
...
EOF
    ...
    print_info "Service installed: imap-tunnel.service"
}
...
#!/bin/bash
# IMAP Tunnel Proxy - Uninstall Script

echo "Stopping service..."
systemctl stop imap-tunnel 2>/dev/null || true
systemctl disable imap-tunnel 2>/dev/null || true

echo "Removing files..."
rm -f /etc/systemd/system/imap-tunnel.service
rm -f /usr/local/bin/imap-tunnel-adduser
rm -f /usr/local/bin/imap-tunnel-deluser
rm -f /usr/local/bin/imap-tunnel-listusers
rm -f /usr/local/bin/imap-tunnel-update
rm -rf /opt/imap-tunnel

echo ""
echo "Note: Configuration in /etc/imap-tunnel was NOT removed"
echo "Remove manually if needed: rm -rf /etc/imap-tunnel"

systemctl daemon-reload

echo ""
echo "IMAP Tunnel Proxy uninstalled successfully"
EOF
...
# IMAP Tunnel Proxy Configuration
...
# IMAP Tunnel Users
# Managed by imap-tunnel-adduser
...
EOF
    ...
    if [ -z "$CREATE_USER" ] || [ "$CREATE_USER" = "y" ] || [ "$CREATE_USER" = "Y" ]; then
        ...
        if [ -n "$FIRST_USER" ]; then
            ...
            cd "$INSTALL_DIR"
            if python3 imap-tunnel-adduser "$FIRST_USER"; then
                echo ""
                ...
                echo "    imap-tunnel-adduser <username>"
            fi
        else
        ...
        echo "    You can create users later with: imap-tunnel-adduser <username>"
    fi
    ...
    if command -v ufw &> /dev/null; then
        if ufw allow 993/tcp >/dev/null 2>&1; then
            print_info "Opened port 993/tcp (ufw)"
        else
            print_warn "Could not configure ufw. Make sure port 993/tcp is open!"
        fi
    elif command -v firewall-cmd &> /dev/null; then
        if firewall-cmd --permanent --add-port=993/tcp >/dev/null 2>&1 && firewall-cmd --reload >/dev/null 2>&1; then
            print_info "Opened port 993/tcp (firewalld)"
        else
            print_warn "Could not configure firewalld. Make sure port 993/tcp is open!"
        fi
    else
        print_warn "No firewall detected. Make sure port 993/tcp is open!"
    fi

    # Enable and start service
    echo ""
    print_step "Starting IMAP Tunnel service..."

    systemctl enable imap-tunnel >/dev/null 2>&1 || true
    systemctl start imap-tunnel 2>&1 || true

    sleep 2

    if systemctl is-active --quiet imap-tunnel; then
        print_info "Service started successfully!"
    else
        print_warn "Service may not have started. Check with:"
        echo "    systemctl status imap-tunnel"
        echo "    journalctl -u imap-tunnel -n 50"
    fi
}
...
print_summary() {
    ...
    echo -e "${GREEN}========================================${NC}"
    echo ""
    echo "Your IMAP Tunnel Proxy is now running!"
    echo ""
    echo -e "${BLUE}Service Status:${NC}"
    echo "   systemctl status imap-tunnel"
    echo ""
    echo -e "${BLUE}View Logs:${NC}"
    echo "   journalctl -u imap-tunnel -f"
    echo ""
    echo -e "${BLUE}User Management:${NC}"
    echo "   imap-tunnel-adduser <username>    Add user + generate client ZIP"
    echo "   imap-tunnel-deluser <username>    Remove user"
    echo "   imap-tunnel-listusers             List all users"
    echo ""
...
main() {
    echo ""
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}  IMAP Tunnel Proxy Installer${NC}"
