#!/usr/bin/env python3
"""
IMAP Tunnel Client - Fast Binary Mode

Version: 1.0.0

Protocol:
1. IMAP handshake (CAPABILITY, STARTTLS, AUTHENTICATE) - looks like real IMAP
2. After AUTHENTICATE, send "BINARY" to switch to streaming mode
...
logger = logging.getLogger('imap-tunnel-client')
...
class TunnelClient:
    ...
    async def connect(self) -> bool:
        """Connect and do IMAP handshake, then switch to binary mode."""
        try:
    ...
    async def _imap_handshake(self) -> bool:
        """Do IMAP handshake then switch to binary."""
        try:
            self._tag_counter = 1
            ...
            # CAPABILITY
            tag = self._next_tag()
            await self._send_line(f"{tag} CAPABILITY")
            if not await self._expect_ok(tag):
                return False

            # STARTTLS
            tag = self._next_tag()
            await self._send_line(f"{tag} STARTTLS")
            line = await self._read_line()
            if not line or not line.startswith(f"{tag} OK"):
                return False

            # Upgrade TLS
            await self._upgrade_tls()

            # CAPABILITY again (post-TLS)
            tag = self._next_tag()
            await self._send_line(f"{tag} CAPABILITY")
            if not await self._expect_ok(tag):
                return False
            ...
            tag = self._next_tag()
            await self._send_line(f"{tag} AUTHENTICATE PLAIN {token}")
            line = await self._read_line()
            if not line:
                logger.error("Auth failed: no response")
                return False
            if line.startswith('+'):
                # Server wants continuation - shouldn't happen with inline token
                await self._send_line(token)
                line = await self._read_line()
            if not line or not line.startswith(f"{tag} OK"):
            ...
            line = await self._read_line()
            if not line or '299' not in line:
    ...
    def _next_tag(self) -> str:
        """Generate next IMAP command tag."""
        tag = f"A{self._tag_counter:03d}"
        self._tag_counter += 1
        return tag
    ...
    async def _expect_ok(self, tag: str) -> bool:
        """Read IMAP responses until we get the tagged response."""
        while True:
            line = await self._read_line()
            if not line:
                return False
            ...
            if line.startswith(f"{tag} OK"):
                return True
            if line.startswith(f"{tag} NO") or line.startswith(f"{tag} BAD"):
                return False
...
def main():
    parser = argparse.ArgumentParser(description='IMAP Tunnel Client (Fast)')
    ...
    config = ClientConfig(
        ...
        server_port=args.server_port or client_conf.get('server_port', 993),
