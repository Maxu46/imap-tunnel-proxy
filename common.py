"""
IMAP Tunnel - Common Protocol and Utilities
...
class TunnelCrypto:
    ...
    def generate_auth_token(self, timestamp: int, username: str = None) -> str:
        """
        Generate authentication token for IMAP AUTHENTICATE.
        ...
        """
        if username:
            message = f"imap-tunnel-auth:{username}:{timestamp}".encode()
            ...
            message = f"imap-tunnel-auth:{timestamp}".encode()
            mac = hmac.new(self.secret, message, hashlib.sha256).digest()
    ...
    def verify_auth_token_multi_user(token: str, users: dict, max_age: int = 300) -> Tuple[bool, Optional[str]]:
        ...
        """
        import logging
        logger = logging.getLogger('imap-tunnel')
        try:
            decoded = base64.b64decode(token).decode()
            parts = decoded.split(':')
...
# ============================================================================
# IMAP Message Generation
# ============================================================================

class IMAPMessageGenerator:
    """
    Generates realistic-looking IMAP messages to wrap tunnel data.
    """
...
class IMAPState:
    """IMAP session state machine for protocol compliance."""
...
def save_users(path: str, users: Dict[str, UserConfig]):
    ...
    """
    lines = ["# IMAP Tunnel Users", "# Managed by imap-tunnel-adduser", "", "users:"]
